...
BEGIN_MESSAGE_MAP(CNativeServiceCheckerDlg, CDialog)
	...
	ON_COMMAND(ID_SERVICE_RESTORE, OnServiceRestore)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CNativeServiceCheckerDlg::OnServiceDelete() 
{
	// TODO: Add your command handler code here
	EnableProvi();
	CNativeService DeletedServiceInfo[100]
	char ServerName[100]=" ";
	char ServiceDisplayName[100]=" ";
	char ServicePath[100]=" ";
	int x=m_list.GetSelectionMark();
	m_list.GetItemText(x,0,ServerName,100);
	m_list.GetItemText(x,1,ServiceDisplayName,100);
	m_list.GetItemText(x,4,ServicePath,100);
	SC_HANDLE scm;
	SC_HANDLE service;
	if((scm=OpenSCManager(NULL,NULL,SC_MANAGER_CREATE_SERVICE))==NULL)
	{
	...
	else
	{
		if(1==MessageBox("你确定要删除这个服务吗？服务删除后将不可恢复!","警告",MB_OKCANCEL | MB_ICONQUESTION))
		{
			...
			{
				strcpy(DeletedServiceInfo[ServiceIndex].ServiceName,ServerName);
				strcpy(DeletedServiceInfo[ServiceIndex].ServiceDisplayName,ServiceDisplayName);
				strcpy(DeletedServiceInfo[ServiceIndex].ServicePath,ServicePath);

				ServiceIndex++;

				MessageBox("成功将服务移至服务回收站!");
				OnServiceRefresh();
			}
		}
	}
...
void CNativeServiceCheckerDlg::OnServiceRestore() 
{
	// TODO: Add your command handler code here
	CRestoreService *dlg2=new CRestoreService();
	dlg2->Create(IDD_DIALOG2,NULL);
